...
public class SalleDashboardController {
    ...
    private void onReservationsPlaceholder() {
        try {
            ...
            ThemeManager.applyTheme(scene);
            Stage stage = (Stage) listPane.getScene().getWindow();
            AppWindow.show(stage, scene, "Skillora Reservations", false);
        } catch (IOException e) {
            showError("Navigation failed", e.getMessage());
        }
    }
    ...
    private void renderSalleCards(List<Salle> salles) throws SQLException {
        ...
        for (Salle salle : salles) {
            cardsContainer.getChildren().add(createSalleCard(salle, resolveLinkedEvent(salle, events, reservations), reservations));
        }
    }

    private VBox createSalleCard(Salle salle, Event linkedEvent, List<Reservation> reservations) {
        ...
        Label placeholder = new Label("Event");
        placeholder.getStyleClass().add("salle-media-placeholder");
        mediaBox.getChildren().add(placeholder);

        String modelSource = safeInput(salle.getImage3d()).trim();
        Image image = loadImage(linkedEvent == null ? null : linkedEvent.getImage());
        if (image != null) {
            ImageView imageView = createCoverImageView(image, 84, 84);
        ...
        heatmapWrap.getChildren().add(createHeatmapGrid(salleReservations, linkedEvent));
    ...
    private HBox createHeatmapGrid(List<Reservation> reservations, Event linkedEvent) {
        HBox wrapper = new HBox(10);
        wrapper.setAlignment(Pos.TOP_LEFT);
        Set<LocalDate> eventDays = resolveEventDays(linkedEvent);
        ...
        for (int i = 0; i < 336; i++) {
            ...
            cell.getStyleClass().addAll("heat-cell", resolveHeatClass(count, eventDays.contains(current)));
    ...
    private Set<LocalDate> resolveEventDays(Event event) {
        ...
        LocalDate end = event.getEndDate() == null ? start : event.getEndDate().toLocalDate();
        ...
        while (!current.isAfter(end)) {
            eventDays.add(current);
            current = current.plusDays(1);
        }
        return eventDays;
    }

    private String resolveHeatClass(int count, boolean eventDay) {
        if (eventDay) {
            return "heat-event";
        }
        if (count <= 0) {
    ...
    private Event resolveLinkedEvent(Salle salle) throws SQLException {
        ...
        return resolveLinkedEvent(salle, eventService.getAll(), reservationService.getAll());
    }

    private Event resolveLinkedEvent(Salle salle, List<Event> events, List<Reservation> reservations) {
        if (salle == null || events == null) {
            return null;
        }

        if (salle.getEventId() != null) {
            Event eventById = findEventById(events, salle.getEventId());
            if (eventById != null) {
                return eventById;
            }
        }
        ...
        for (Event event : events) {
            if (event.getId() != null && event.getSalleId() != null && event.getSalleId().equals(salle.getId())) {
                return event;
            }
        }
        ...
        for (Reservation reservation : reservations) {
            if (reservation.getSalleId() != null
                    && reservation.getSalleId().equals(salle.getId())
                    && reservation.getEventId() != null) {
                Event eventById = findEventById(events, reservation.getEventId());
                if (eventById != null) {
                    return eventById;
                }
            }
    ...
    private Event findEventById(List<Event> events, Integer eventId) {
    ...
    private void configureAdminSidebar() {
        if (adminSidebarController != null) {
            adminSidebarController.setActive(AdminSidebarController.ActiveItem.SALLES);
        }
    }
