...
public class EventDashboardController {
    ...
    public void initialize() {
        ...
        searchField.textProperty().addListener((obs, oldValue, newValue) -> resetEventsPaginationAndLoad());
        typeFilterCombo.valueProperty().addListener((obs, oldValue, newValue) -> resetEventsPaginationAndLoad());
        sortCombo.valueProperty().addListener((obs, oldValue, newValue) -> resetEventsPaginationAndLoad());
    ...
    @FXML
    private void onPreviousEventsPage() {
    ...
    private void onOpenSalles() {
        try {
            ...
            AppWindow.show(stage, scene, "Skillora Salles", false);
        } catch (Exception e) {
    ...
    private void resetEventsPaginationAndLoad() {
        currentEventsPage = 1;
        loadEvents();
    }
    ...
    private void renderRows(List<Event> events) {
        ...
        int totalPages = Math.max(1, (int) Math.ceil(totalEvents / (double) EVENTS_PER_PAGE));
        currentEventsPage = Math.max(1, Math.min(currentEventsPage, totalPages));
        int fromIndex = Math.min((currentEventsPage - 1) * EVENTS_PER_PAGE, totalEvents);
        int toIndex = Math.min(fromIndex + EVENTS_PER_PAGE, totalEvents);
        List<Event> pageEvents = totalEvents == 0 ? List.of() : events.subList(fromIndex, toIndex);

        for (Event event : pageEvents) {
            cardsContainer.getChildren().add(createEventRow(event));
        }

        resultCountLabel.setText(totalEvents + " event(s)");
        updatePaginationControls(totalPages);
    }

    private void updatePaginationControls(int totalPages) {
        if (previousPageButton != null) {
            previousPageButton.setDisable(currentEventsPage <= 1);
        }
        if (nextPageButton != null) {
            nextPageButton.setDisable(currentEventsPage >= totalPages);
        }
        if (pageStatusLabel != null) {
            pageStatusLabel.setText("Page " + currentEventsPage + " / " + totalPages);
        }
        ...
        int firstPage = Math.max(1, currentEventsPage - 2);
        ...
        for (int page = firstPage; page <= lastPage; page++) {
            ...
            if (page == currentEventsPage) {
                pageButton.getStyleClass().add("pagination-page-button-active");
            }
            ...
            pageNumbersBox.getChildren().add(pageButton);
        }
    }
    ...
    private void setActiveNav(Button activeButton) {
        configureAdminSidebar();
        if (eventsNavButton == null || activeButton == null) {
            return;
        }
    ...
    private void configureAdminSidebar() {
        if (adminSidebarController != null) {
            adminSidebarController.setActive(AdminSidebarController.ActiveItem.EVENTS);
        }
    }
    ...
    private void openSkillHarborEvents() {
        try {
            ...
            ThemeManager.applyTheme(scene);
            Stage stage = (Stage) searchField.getScene().getWindow();
            AppWindow.show(stage, scene, "SkillHarbor Events", false);
        } catch (Exception e) {
            showError("Navigation impossible", e.getMessage());
        }
    }
